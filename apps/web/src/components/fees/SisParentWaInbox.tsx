...
export function SisParentWaInbox({
  by,
  canEdit = true,
}: {
  by: string;
  ...
  const refresh = useCallback(async () => {
    try {
      const res = await fetch("/api/wa/sis-bot/threads");
      if (!res.ok) return;
  ...
  const keywords = SIS_BOT_QUICK_PROMPTS.map((q) => q.waKeyword).join(" · ");
  ...
  return (
    <div className="space-y-4">
      <p className="text-[12px] text-[var(--muted)]">
        WhatsApp bot for <strong>SIS enrolled parents</strong> (registered
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <MastersWorkCard
          title={
            selected
              ? `WhatsApp · ${selected.parentName || selected.mobile}`
              : "Select a thread"
          }
              ...
              {canEdit ? (
                <>
                  <textarea
                    ...
                    placeholder="Reply as school office…"
                    value={reply}
