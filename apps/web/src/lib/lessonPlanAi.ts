...
export function buildLessonPlanSystemPrompt(opts: {
  ...
  const grounding = outcomes
    ? `
- Pre-primary: the school teaches from its own publisher books; when they are listed at the end (unit by unit, no chapter numbers), plan within the unit the ticked items belong to and name it as listed — never a chapter number. NCERT's learning outcomes for this year, also listed at the end, are the minimum a child should reach — not the syllabus and not a ceiling. Plan play-based, hands-on activities (objects, songs, stories, drawing, movement) for what the ticked units ask, building towards the outcomes they touch; where the unit goes beyond them, follow the unit. Name the outcome in plain words in an objective when one clearly fits — never an outcome code, and never an NCERT book or chapter: pre-primary has none.`
    : textbooks
      ? `
