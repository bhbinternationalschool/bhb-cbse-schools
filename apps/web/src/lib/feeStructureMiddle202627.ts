/**
 * BHB Middle (Classes VI–VIII) fee structure — session 2026-27.
 * Source: school fee PDF (new ₹32,300 · old/promoted ₹27,300; transport extra).
 */
...
export const MIDDLE_FEE_AY = "2026-27";

const NEW_GROUP_CODE = "NEW_MIDDLE_2627";
const PROMOTE_GROUP_CODE = "PROMOTE_MIDDLE_2627";

const MIDDLE_CLASS_NAMES = ["VI", "VII", "VIII"];
...
function ensureAcademicYear202627(state: MastersState): MastersState {
  ...
  if (years.some((y) => y.code === MIDDLE_FEE_AY)) return state;
...
function middleNewPlan(): Record<string, MonthCharges> {
  const plan: Record<string, MonthCharges> = {
    ...
    SEP: { TUITION: 1900, EXAM: 500 },
    FEB: { TUITION: 1900, EXAM: 500 },
  };
...
function middlePromotePlan(): Record<string, MonthCharges> {
  const plan: Record<string, MonthCharges> = {
    APR: {
      AMENITY: 2000,
      COMMUNICATION: 500,
...
export function ensureMiddleFeeStructure202627(
  state: MastersState,
): MastersState {
  const hasGroups = (state.feeGroups ?? []).some(
    (g) =>
      g.academicYearCode === MIDDLE_FEE_AY &&
      (g.code === NEW_GROUP_CODE || g.code === PROMOTE_GROUP_CODE),
  );
  if (hasGroups) return state;

  const classIds = classIdsForBand(state, MIDDLE_CLASS_NAMES);
  if (!classIds.length) return state;

  let next = ensureAcademicYear202627(state);
  next = ensureAprToMarInstallments(next, MIDDLE_FEE_AY);
  ...
  for (const i of next.installments) {
    if (i.academicYearCode === MIDDLE_FEE_AY) {
      instId[i.code] = i.id;
    }
  }
  ...
  const newGroup: FeeGroup = {
    ...
    academicYearCode: MIDDLE_FEE_AY,
    studentType: "NEW",
    classIds,
    isActive: true,
  ...
  return {
    ...
    feeStructureLines: [
      ...
      ...buildLines(newGroupId, headId, instId, middleNewPlan()),
      ...buildLines(promoteGroupId, headId, instId, middlePromotePlan()),
    ],
  };
}
...
export function middleFeeStructureTotals(state: MastersState): {
  newRupees: number;
