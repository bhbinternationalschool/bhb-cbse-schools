...
function defaultSettings(): AccountsSettings {
  return {
...
function normalizePool(p: Partial<CashPool>): CashPool {
  return {
    id: p.id ?? id("pool"),
...
function normalizeBank(b: Partial<BankAccount>): BankAccount {
...
function syncModeBankMapFromBanks(
  bankAccounts: BankAccount[],
): ModeBankMapEntry[] {
...
function normalizeExpenseVoucherLine(
...
function normalizeRule(r: Partial<RecurringExpenseRule>): RecurringExpenseRule {
  return {
    id: r.id ?? id("rec"),
...
function normalizeVendor(v: Partial<AccountsVendor>): AccountsVendor {
  return {
    id: v.id ?? id("ven"),
    name: v.name ?? "Vendor",
...
function normalizePayable(p: Partial<AccountsPayable>): AccountsPayable {
  return {
    id: p.id ?? id("pay"),
    vendorId: p.vendorId ?? "",
...
function normalizeTrustee(t: Partial<Trustee>): Trustee {
  return {
    id: t.id ?? id("trs"),
    name: t.name ?? "Trustee",
    phone: t.phone ?? "",
...
function normalizeLoan(l: Partial<OwnerLoan>): OwnerLoan {
  return {
    id: l.id ?? id("oln"),
...
function normalizeLoanRow(r: Partial<OwnerLoanScheduleRow>): OwnerLoanScheduleRow {
  return {
    id: r.id ?? id("olr"),
    loanId: r.loanId ?? "",
...
function normalizeCoa(c: Partial<CoaAccount>): CoaAccount {
  return {
    id: c.id ?? id("coa"),
    code: c.code ?? "0000",
    name: c.name ?? "Account",
...
function normalizeBankLedger(e: Partial<BankLedgerEntry>): BankLedgerEntry {
  return {
    id: e.id ?? id("ble"),
    bankId: e.bankId ?? "",
    date: e.date ?? todayIso(),
...
function normalizeJournal(j: Partial<JournalEntry>): JournalEntry {
  return {
    id: j.id ?? id("jv"),
...
function normalizeReconSession(s: Partial<ReconSession>): ReconSession {
  return {
    id: s.id ?? id("recn"),
    bankId: s.bankId ?? "",
...
function repairOrphanedCancelledVoucherLedger(
  state: AccountsState,
): AccountsState {
