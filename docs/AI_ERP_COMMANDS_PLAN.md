...
| 2026-09-05 | **Book a PTM slot for a family.** `book_ptm` — "Book PTM slot for Riya Verma, 10:30", "book ptm for Aarav Sharma 11 am", "Amay Gupta 4B ka PTM 10:30 book karo", "fix ptm appointment for Kabir Ali". For the parent who rings the office instead of using the portal. Needs the PTM word **and** a booking word, so "PTM kab hai" stays a question; it is parsed after every colon-shaped notice command, so "message 5A parents: PTM slot booking is open" stays a notice. A time needs a separator or a marker (`10:30`, `10.30 am`, `2 pm`, `11 baje`) — a bare number is a class or a roll, never an hour — and an hour written without am/pm is matched against the slots that **actually exist** (`2:30` tries 02:30 and 14:30) rather than guessed. No time, or a time whose slot is taken, lists the open ones instead of booking something else. A child already booked on that event is reported, not double-booked. New `lib/ptmBook.server.ts` holds the shared path: it picks the nearest open PTM covering the child's class, and `POST /api/v1/ptm/book` — parent-only, unchanged — now calls it too, so a staff booking is the same record as a parent's, through the same `bookPtmSlot` validator. The booking is recorded under **the parent's name**, not the staff member's. The family is told on the parent app, and on WhatsApp as the approved notice template when one is approved; with none the card says the family is told on the app only rather than implying a message. Class teachers book for their own sections, office for any, re-checked at confirm. |
| 2026-09-05 | **Bug found and fixed while here.** `looksLikeCommand`, the gate deciding whether an unmatched WhatsApp message is worth a model parse, used `\b` — which is ASCII-only, so the Devanagari half of its word list (कौन, कितना, बताओ, दिखाओ, फीस, बकाया) could never match and a Hindi-script command phrased outside the regexes was silently dropped. Now Unicode lookarounds, with tests. (The list itself stays as narrow as it was — `book_ptm` is matched by regex, so PTM chatter never buys a model call.) |
| 2026-09-05 | **Bus delay notice — Phase 2 complete.** `bus_delay` — "Bus 3 ko batao: 20 minute late", "bus 5 is running 15 mins late", "route A 20 minute late", "bus 2 aadha ghanta late hai", "बस 3 आधा घंटा लेट". Like the fee reminder this is **one templated message per family**, not a broadcast: each parent gets their own child's name and their own stop, through the approved **Bus running late** utility template. **The delay must carry a unit** — the bus number and the minutes are both bare numbers, so "bus 3 20 late" asks how late rather than guessing which number is which; "half an hour" / "aadha ghanta" / "1 hour" are understood, and anything over three hours is refused as a typo with the advice to ring the families instead. Riders are derived exactly as the manifest derives them (live assignments, then the latest year that actually has riders), so the card lists the same children as the driver's sheet, grouped by stop. Two skips are shown rather than silent: a child **suspended from boarding** is not on the bus today, and a family who sent STOP is not messaged. **No quiet hours** — unlike a fee chase, a parent standing at a dark stop is the reason this message exists — but the card warns when the same route was told within the hour, so a second notice is a decision rather than an accident (a bus slipping 20 → 40 minutes must still be able to say so, so it is a warning, not a cap). New `lib/busDelayNotice.server.ts` plans and sends; the route matcher the manifest reading used inline moved to a shared `matchTransportRoutes`, so a bus name that resolves for one command resolves identically for the other. Transport desk, office and leadership only, re-checked at confirm. |
| 2026-09-05 | **Class-channel ERP-write gap closed.** When a teacher WhatsApped "HW 6B maths page 42" and replied YES, parents were messaged in that instant but the ERP record was written only the next time somebody opened Comms → Class Channels in a browser — `applyClassChannelDraftToErp` was called from a React effect. Parents had the homework; the homework desk might not, for hours, or for good if nobody opened that screen. The write now happens where the confirmation happens: `applyDraftToErpServer` runs inside `confirmClassChannelDraft`, through `postHomeworkServer` (already shared with the v1 route and the `post_homework` command) and a new `publishNoticeServer` — the notice equivalent, built on the loader and persist the scheduled-publish tick already used, because `upsertNotice` checks a browser session's permission and saves to localStorage. **No double write:** a draft applied server-side is marked `applied`, and the browser panel only ever applied drafts still marked `confirmed`; its post-confirm call now runs solely when the server reports a failure, so it stays a real fallback without filing anything twice. The decision of *what* a draft becomes — homework or notice, which audience, whether pinned, where the event date and the attachment note go — moved into one pure `classChannelErpFields`, used by both appliers and covered by a new `test:class-channel-apply`, so the browser and the server cannot word the same post differently. Both the teacher's WhatsApp reply and the panel's banner now say what actually reached the ERP: the reply used to promise "ERP will pick this up" and the banner used to print "ERP updated" whether or not anything had. |
| 2026-09-07 | **Rebased onto main** (39 commits of it), which is what `ratchets` had been red about: this branch was carrying a stale `scripts/ratchets.txt` with budgets main had since raised, so two counters read over budget here and in budget there. Every counter is now green. Four conflicts, all resolved keeping **both** sides: main's `assertSectionScope` guard on `/api/v1/homework/post` and `/api/v1/attendance/mark` (added after this branch forked — a teacher could otherwise post to any section) now sits in front of the shared server helpers those routes were refactored onto; main's staff-session guard on the class-channel route (it had been listing every section's parent mobiles to anyone) keeps the new `erp` field in its confirm response; and the mobile home screens keep both the command bar and main's waiting card. Also registered `test:class-channel-apply` in `.github/workflows/ci.yml` — it was in `verify.sh` and `package.json` but not CI, which the suite's own orphan and drift checks caught. |
| next | Phase 2's ten commands are done, the class-channel gap is closed, and the branch is current with main. Phase 3 — or an end-to-end run on real data with the director. |
