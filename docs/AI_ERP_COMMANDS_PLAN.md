...
| next | Phase 2 write commands, in the plan's order: post homework, mark attendance, class parent message, staff broadcast, raise complaint, approve leave, fee reminder, payment link, book PTM, bus delay notice. Each needs the confirm card (already built) and, for the last five, a server function first. |
