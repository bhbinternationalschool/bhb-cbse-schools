...
| Human-in-the-loop | 🟡 | All staff-facing generators land in an editor before save/print ✅ and report accepted / edited / rejected to `ai_generations` ✅. **Exception: WA bot LLM fallback auto-replies to parents** — the prompt already restricts it to household data + notices and defers to *HUMAN*; a hard confidence gate is still to do |
