...
| Missing | Types CBSE now weights: **case-study / source-based, assertion-reasoning, competency (application/HOTS)**. No competency tag on a question. No marking scheme (only a one-line answer key). No syllabus-chapter linkage — questions are generated from class+subject only, not from `SyllabusUnit` chapters/topics |
...
| Data needed | **CBSE competency / LO codes** — `SyllabusUnit.learningOutcomes` is free text today; needs a structured `competencyCodes: string[]` per unit and a seed table of CBSE LOs per class×subject (one-time import; NCERT LO documents are public) |
