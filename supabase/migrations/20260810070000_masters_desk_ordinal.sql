...
-- differing: the reader sorted by (sort_order, id) while the slice holds
-- whatever order it was written in. That order is what staff see today —
-- lists render in array order — and while 29 call sites do sort by sortOrder,
-- plenty use .find() and nothing guarantees the rest sort.
--
-- Auditing every consumer of twenty collections to decide whether order
-- matters is a real audit, and guessing is how a "safe" switch quietly
-- reorders somebody's class list. Capturing the original position makes the
-- question moot: the reader reproduces the slice exactly, and reordering
-- anything later becomes a separate, visible change.
--
...
begin
  ...
  loop
    execute format(
      'alter table public.%I add column if not exists ordinal integer not null default 0', r.tbl);
    execute format($f$
      update public.%I t set ordinal = src.idx
        from (select e->>'id' as id, (ord - 1)::int as idx
                from public.masters_desk_slices s,
                     jsonb_array_elements(s.payload) with ordinality as a(e, ord)
               where s.slice_key = %L) src
       where t.id = src.id
    $f$, r.tbl, r.slice_key);
  end loop;
end $$;
