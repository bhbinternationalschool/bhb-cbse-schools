-- Repair: six more tables still pointed at pre-re-seed masters ids.
--
-- Same cause as 20260809100000 (which fixed student class/section):
-- masters were re-seeded into masters_desk_slices with freshly generated
-- ids, while rows written before the cutover kept the ids from the
-- school_mirror_state blob. A sweep of every normalized table for
-- cls_/sec_/cam_/fg_ tokens found these still dangling:
--
--   sis_students.campus_id            711 rows  (1 campus)
--   sis_students.fee_group_id         481 rows  (12 fee groups)
--   rte_desk_seats.class_id            10 rows
--   ptm_desk_events.class_ids_json      1 row   (3 class ids)
--   homework_desk_posts                 1 row   (class + section)
--   homework_desk_diary                 1 row   (class + section)
--   wa_desk_bot_slices 'classChannel'  15 channels (class + section)
--
-- Every one of these resolves against the blob masters by name, so the
-- mapping is name-based and 1:1, exactly as in the earlier repair.
--
-- Deliberately NOT touched here:
--   * admission_desk_leads.class_sought_id — those ids resolve against
--     neither the desk nor the blob (a third, extinct generation). It is
--     recovered from import-note text in 20260809120000, kept separate so
--     an inference-based repair can be reverted on its own.
--   * school_mirror_state.masters — it is the mapping source for this
--     migration and must stay as-is.
--
-- Snapshots taken before applying: <table>_pre_idremap_20260809.
--
-- Idempotent: once ids already resolve, every statement matches no rows.

-- ── Mapping table: blob id → desk id, by name ────────────────────────
...
-- ── sis_students.campus_id / fee_group_id ───────────────────────────
...
-- ── rte_desk_seats.class_id ─────────────────────────────────────────
update public.rte_desk_seats r
   set class_id = m.new_id, updated_at = now()
  from public._blob_to_desk_id_map m
 where m.kind = 'class' and m.old_id = r.class_id;

-- ── homework_desk_posts / _diary (class + section) ───────────────────
-- Scalar subqueries, not joins: an empty section_id must leave the
-- section alone, not match every row in the section map and pick one
-- of them arbitrarily.
...
-- ── ptm_desk_events.class_ids_json (array of class ids) ─────────────
...
-- ── wa_desk_bot_slices 'classChannel' ───────────────────────────────
-- Channel ids are derived as ch_<ay>_<sectionId> by syncClassChannels,
-- so the id is rewritten alongside the section it encodes; leaving it
-- stale would make the next sync create duplicate channels rather than
-- match these. drafts/threads are empty, so nothing references the old
-- channel ids.
