-- Replace masters_sync_rows_from_slices's per-row EXECUTE loop with a
-- set-based batch per slice type. Same 21-entity reconciliation, same
-- snake-casing via _masters_snake, same "only real, known columns; an
-- unknown key is ignored rather than erroring" tolerance, same upsert +
-- delete-stale semantics, same masters_desk_settings handling for the
-- three single-document slices -- only the *mechanism* changes.
--
-- Why: the original built and EXECUTEd a brand-new SQL string per item
-- (one per class, per section, per fee head, ... across 21 categories).
-- Because the SQL text itself differs every iteration (the column list is
-- recomputed from that row's own keys), Postgres can never reuse a cached
-- plan across iterations -- every single row pays a full parse+plan, which
-- is strictly worse than a fixed-shape per-row loop. In production this
-- was timing out ("[masters] row-table sync failed ... statement
-- timeout"), and very likely starving unrelated reads on the same
-- database too (e.g. admissions fetches timing out with nothing else
-- explaining it) by holding the connection/CPU busy for so long per push.
--
-- Now: for each of the 21 slice types, one set-based query builds every
-- row's snake-cased + enriched (tenant_id/ordinal/updated_at) JSONB object
-- in a single pass, one query determines the real-column union for that
-- batch, and one dynamic INSERT ... SELECT ... ON CONFLICT (still built
-- via EXECUTE format(), since the column list is genuinely
-- data-dependent) applies the whole batch — so the per-slice-type cost is
-- now O(1) EXECUTE calls instead of O(items in that slice). The
-- delete-stale-by-not-in-ids step and the single-document settings
-- handling are unchanged (they were already set-based).
