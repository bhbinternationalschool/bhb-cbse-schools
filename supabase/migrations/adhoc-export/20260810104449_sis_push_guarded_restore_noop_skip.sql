...
-- 20260808150000 is RECORDED as applied (twice) but the live function has no
-- skip: it checks the base revision then writes unconditionally. The file was
-- edited after it ran, and a recorded version is never re-run. Full rationale
-- in supabase/migrations/20260810110000_sis_push_guarded_restore_noop_skip.sql.
...
begin
  ...
  values (
    ...
    (select count(*) from public.sis_students   from_dummy_placeholder_removed where tenant_id = p_tenant_id),
    active_students,
    now_ts
  )
