...
--
-- The Stage 2.1/2.2 audit found the database holds TWO schemas: an
-- abandoned relational design (~25 tables, uuid ids, fully FK-linked, all
-- empty, zero code references) and the one the app actually runs on (text
-- ids like cls_p7bw8cpc, in sis_* and *_desk_* tables). classes.id is uuid
-- there; every class id here is text. Adapting it would mean changing id
-- types on 20 tables, dropping 15 FK constraints, and reconciling
-- students/sis_students, concession_rules/concessions and
-- fee_installments/installments. That is a schema merge, not this stage.
--
-- So: build fresh, named masters_desk_* to sit beside masters_desk_slices,
-- and leave the abandoned schema entirely alone.
--
-- No cross-table foreign keys yet. The desk tables the app already uses
-- mostly do without them, and adding them here would fail on any pre-existing
-- referential drift in the JSONB before we have measured it. Integrity is
-- checked in the verification step instead.
...
-- The three object slices (schoolProfile, schoolTiming, midYearFeePolicy)
-- are single documents, not collections. One settings row each.
