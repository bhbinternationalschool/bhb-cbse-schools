...
-- row-by-row in plpgsql over 904 records and measured 16.7s in production, so
-- it never completed. pushSisGuarded() treats ANY rpc error as "guarded push
-- unavailable" and falls back to a legacy last-write-wins upsert — the exact
...
-- loop is still O(n) round trips inside plpgsql, and at several thousand
