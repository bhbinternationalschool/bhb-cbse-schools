...
-- reconciled on every push, so they can never become a second diverging
-- source of truth.
--
-- Direction matters. The slices stay authoritative; the rows follow. Writing
-- both independently would mean two writers and a merge problem, which is
-- the disease being cured rather than a treatment for it.
--
...
-- row table — a phantom class, which is how orphaned references start. An
-- absent slice key empties its table for the same reason.
--
-- Verified: a plain re-sync reports 0 deletions (no drift); a planted
-- phantom class is removed on the next sync; adding then dropping a class
-- through the real write path leaves both stores at 15 with the probe absent
-- from each; parity holds afterwards.
...
declare
  pair record; item jsonb; ord int; snake jsonb; cols text; sets text;
  ids text[]; n_up int := 0; n_del int := 0; removed int;
  report jsonb := '{}'::jsonb;
begin
  ...
  loop
    ids := array[]::text[]; ord := 0;

    for item in
      select e from public.masters_desk_slices s, jsonb_array_elements(s.payload) e
       where s.tenant_id = p_tenant_id and s.slice_key = pair.slice_key
    loop
      if nullif(item->>'id','') is null then continue; end if;
      ids := ids || (item->>'id');

      -- camelCase -> column names, so 21 entity mappings need not be written
      -- by hand and cannot drift from each other. `ordinal` carries the array
      -- position so the reader reproduces the slice's order exactly.
      select coalesce(jsonb_object_agg(public._masters_snake(k), v), '{}'::jsonb)
        into snake from jsonb_each(item) as kv(k, v);
      snake := snake || jsonb_build_object('tenant_id', p_tenant_id,
                          'ordinal', ord, 'updated_at', clock_timestamp());

      -- Only real columns; an unknown key is ignored rather than erroring,
      -- so a new field in the slice cannot break the sync.
      select string_agg(format('%I', k), ', '),
             string_agg(format('%I = excluded.%I', k, k), ', ')
        into cols, sets
        from jsonb_object_keys(snake) k
       where exists (select 1 from information_schema.columns c
          where c.table_schema='public' and c.table_name=pair.tbl and c.column_name=k)
      ...
      n_up := n_up + 1; ord := ord + 1;
    end loop;

    execute format('delete from public.%I where tenant_id = $1 and not (id = any($2))',
